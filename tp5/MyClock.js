 ...
 */
 function MyClock(scene, slices, stacks) {
	...
	this.horas = new MyClockHand(scene, 4, 0.4);
	this.horas.initBuffers();
	this.minutos = new MyClockHand(scene, 3, 0.6);
	this.minutos.initBuffers();
	this.segundos = new MyClockHand(scene, 2, 0.8);
...
MyClock.prototype.display = function() {
    ...
    this.horas.setAngle(this.hora * 30);
	this.horas.display();
	this.scene.popMatrix();
	...
	this.scene.translate(0, 0, 1);
    this.materialH.apply();
    this.minutos.setAngle(this.minuto * 6);
